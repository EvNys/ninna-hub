      ...
      <section className="bg-brand-darker py-24 px-8 rounded-[40px] m-4 md:m-8 overflow-hidden relative shadow-2xl">
        ...
        <div className="max-w-4xl">
          ...
          <p className="text-gray-400 font-medium text-lg italic">
            Conecte-se com especialistas prontos para ajudar sua startup a
            superar desafios técnicos e de mercado.
          </p>
        </div>
      </section>
          ...
          {loading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
                <div
                  key={i}
                  className="aspect-[3/4] bg-white animate-pulse rounded-[40px] border border-gray-100"
                />
              ))}
            </div>
          ) : (
              ...
              {filtered.map((mentor, i) => (
                ...
                >
                      ...
                      <div className="flex gap-4">
                        {mentor.linkedin && (
                          <a
                            ...
                            className="w-12 h-12 bg-white/10 backdrop-blur-sm rounded-2xl flex items-center justify-center hover:bg-brand-teal transition-all"
                          >
                  ...
                  <div className="p-8 text-center bg-white">
                    <h3 className="text-xl font-black text-gray-900 uppercase tracking-wide italic mb-1 px-4">
                      {mentor.nome}
                    </h3>
                    <p className="text-brand-teal font-black text-[10px] uppercase tracking-widest mb-1 italic px-2">
                      {mentor.cargo}
                    </p>
                    <p className="text-gray-400 font-bold text-[10px] uppercase tracking-widest italic">
                      {mentor.empresa}
                    </p>
                  </div>
                </motion.div>
