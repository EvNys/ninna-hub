...
export default function AdminInscricoesMentores() {
  ...
  const fetchInscricoes = async () => {
    ...
    try {
      const q = query(
        collection(db, "inscricoes_mentores"),
      ...
      if (selectedInscricao) {
        const updatedSelected = data.find(
          (item) => item.id === selectedInscricao.id,
        );
        if (updatedSelected) {
  ...
  const handleUpdateStatus = async (
    id: string,
    newStatus: "pendente" | "apto á começar o programa" | "inapto",
  ) => {
    try {
  ...
  const handleDeleteInscricao = async (id: string) => {
    if (
      window.confirm(
        "Tem certeza de que deseja excluir permanentemente esta inscrição?",
      )
    ) {
      try {
  ...
  const handlePromoteToRoster = async (item: any) => {
    if (
      window.confirm(
        `Deseja aprovar e cadastrar ${item.nome} diretamente no Roster Oficial de Mentores?`,
      )
    ) {
      try {
        ...
        toast.success(
          `${item.nome} promovido com sucesso ao Roster Oficial de Mentores de Inovação!`,
        );
        fetchInscricoes();
      } catch (error) {
        console.error(error);
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-10 gap-6 pb-6 border-b border-gray-100">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-2">
            Acompanhe o funil de recebimento e aprove os profissionais para o
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-2 bg-white border border-gray-100 rounded-[32px] overflow-hidden shadow-lg">
          ...
          <div className="p-4 bg-[#fafafa] border-t border-gray-50 flex items-center justify-between text-xs text-gray-400 font-barlow">
            <span>
              Listando {filteredInscricoes.length} de {inscricoes.length}{" "}
          ...
          <AnimatePresence mode="wait">
            {selectedInscricao ? (
              <motion.div
                ...
                exit={{ opacity: 0, y: 15 }}
                className="bg-white border border-gray-100 p-8 rounded-[32px] shadow-xl text-left space-y-6 relative"
              >
                {/* Profile Header */}
                <div className="flex items-start justify-between border-b border-gray-100 pb-5">
                  <div className="min-w-0">
                    <span className="text-[9px] font-black text-brand-teal uppercase tracking-widest block mb-1">
                      Candidato Escolhido
                    </span>
                    <h3 className="font-extrabold uppercase text-lg text-gray-900 tracking-wide leading-snug break-words">
                      {selectedInscricao.nome}
                    </h3>
                    <p className="text-gray-400 text-xs font-barlow truncate mt-0.5">
                      {selectedInscricao.email}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4.5 bg-gray-50 rounded-2xl border border-gray-100 space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <button
                        onClick={() =>
                          handleUpdateStatus(selectedInscricao.id, "pendente")
                        }
                      ...
                      <button
                        onClick={() =>
                          handleUpdateStatus(selectedInscricao.id, "inapto")
                        }
                        className={`w-full py-2.5 rounded-xl font-bold uppercase text-[10px] tracking-widest border transition-all text-center flex items-center justify-center gap-1.5 ${
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="p-4 bg-gray-50 rounded-2xl border border-gray-100/50 text-xs text-gray-500 font-barlow space-y-2">
                      ...
                      <div className="flex items-center gap-2.5">
                        <MapPinned className="w-4 h-4 text-brand-teal shrink-0" />{" "}
                        {selectedInscricao.pais}
                      </div>
                      <div className="flex items-center gap-2.5">
                        <MapPin className="w-4 h-4 text-brand-teal shrink-0" />{" "}
                        {selectedInscricao.estado}
                      </div>
                      <div className="flex items-center gap-2.5">
                        <MapPinHouse className="w-4 h-4 text-brand-teal shrink-0" />{" "}
                        {selectedInscricao.cidade}
                      </div>
                      <div className="flex items-center gap-2.5">
                        <Calendar className="w-4 h-4 text-brand-teal shrink-0" />{" "}
                        Inscrito em {formatDate(selectedInscricao.createdAt)}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1">
                    ...
                    <p className="text-gray-600 font-barlow text-xs leading-normal">
                      {selectedInscricao.comoConheceu}
                    </p>
                  </div>
                  ...
                  <div className="text-[10.5px] font-bold text-gray-500 flex items-center gap-2 bg-green-50/50 p-2.5 rounded-lg border border-green-100/30">
                    ...
                    Ciente e concorda com o regulamento.
                  </div>
                </div>
              ...
              <div className="border-2 border-dashed border-gray-100 p-8 py-32 rounded-[32px] text-center text-gray-400 flex flex-col items-center bg-white">
                ...
                <span className="font-extrabold uppercase tracking-widest text-[10px] leading-relaxed max-w-[200px] text-gray-300">
                  Selecione um candidato cadastrado na tabela ao lado para
                  conferir os dados detalhados e atualizar o status
                </span>
              </div>
            )}
          </AnimatePresence>
