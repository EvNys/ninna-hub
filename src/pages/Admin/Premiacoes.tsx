...
const AdminPremiacoes = () => {
  const { items, loading, refetch, create, update } =
    useFirestoreCollection<Premiacao>({
  ...
  const handleDelete = async (id: string) => {
    ...
    toast.promise(promise, {
      ...
      success: () => {
        refetch();
        setIsDeleting(null);
        return "Premiação excluída com sucesso";
      },
    ...
    <div className="max-w-7xl mx-auto py-12">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, organizacao: e.target.value })
              }
              className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-6 py-4 focus:outline-none focus:border-brand-teal transition-all text-gray-900 shadow-sm"
            />
          </div>
        ...
        {isDeleting && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              ...
              <h2 className="text-3xl font-black text-gray-900 uppercase tracking-wide italic mb-4">
                Excluir Premiação?
              </h2>
              <p className="text-gray-500 mb-10 font-medium">
                Esta ação não pode ser desfeita. A premiação será removida
                permanentemente do portal.
              </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          ...
          <AdminEmptyState
            icon={
              <Award className="w-16 h-16 mb-6 opacity-20 text-brand-teal" />
            }
            ...
            >
              <div className="relative h-48 overflow-hidden bg-gray-50">
                ...
                <div className="absolute top-4 right-4 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-3 bg-white/95 text-gray-900 hover:text-brand-teal rounded-2xl shadow-xl transition-all"
                  >
              ...
              <div className="p-10">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-brand-teal text-[10px] font-black uppercase tracking-widest">
                    {item.ano}
                  </span>
                  {item.status === "inativo" && (
                    <span className="text-red-400 text-[8px] font-black uppercase tracking-widest border border-red-100 px-2 py-0.5 rounded-full">
                      Inativo
                    </span>
                  )}
                </div>
                <h3 className="text-2xl font-black text-gray-900 uppercase tracking-wide italic leading-none mb-4">
                  {item.titulo}
                </h3>
