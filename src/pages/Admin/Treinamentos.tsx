...
import { Plus, Trash2, Edit2, PlayCircle, Save, X, GraduationCap, Link2, Layout, ArrowLeft, Eye, EyeOff } from 'lucide-react';
...
const AdminTreinamentos = () => {
  ...
  const categories = ["Marketing", "Vendas", "Operacional", "Estratégico", "Jurídico", "Outros"];
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-2 italic">Publique cursos e aulas exclusivas para a comunidade.</p>
        </div>
            ...
            >
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Título do Treinamento</label>
                  <input 
                    required 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          [1, 2, 3].map(i => <div key={i} className="bg-white h-72 animate-pulse rounded-[40px] border border-gray-100" />)
        ) : trainings.map(item => (
          <motion.div key={item.id} className="bg-white rounded-[40px] border border-gray-100 p-8 shadow-xl relative group">
            <div className="aspect-video bg-gray-50 rounded-[30px] mb-6 overflow-hidden relative">
              ...
              <div className="absolute top-4 right-4 flex gap-2 z-10">
                ...
                <button 
                  type="button"
                  onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleStatus(item); }} 
                  ...
                  title={item.status === 'ativo' ? "Desativar (Ocultar)" : "Ativar (Mostrar)"}
                >
              ...
              {item.status === 'inativo' && (
                <div className="absolute inset-0 bg-gray-900/40 backdrop-blur-[2px] flex items-center justify-center pointer-events-none">
                  <span className="bg-white/10 text-white border border-white/20 px-4 py-2 rounded-full font-black text-[10px] uppercase tracking-widest backdrop-blur-md">Inativo</span>
                </div>
              )}
            </div>
            
            <div className="flex items-center gap-2 mb-3">
              <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border ${item.status === 'ativo' ? 'bg-brand-teal/5 text-brand-teal border-brand-teal/10' : 'bg-gray-100 text-gray-400 border-gray-200'}`}>
                {item.status}
              </span>
