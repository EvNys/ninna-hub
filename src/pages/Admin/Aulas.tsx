...
const AdminAulas = () => {
  ...
  const fetchTraining = async () => {
    try {
      ...
      if (docSnap.exists()) {
        setTraining({ id: docSnap.id, ...docSnap.data() } as Treinamento);
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const fetchLessons = async () => {
    try {
      const q = query(
        collection(db, "treinamentos", trainingId!, "aulas"),
        orderBy("ordem", "asc"),
      );
      const querySnapshot = await getDocs(q);
      setLessons(
        querySnapshot.docs.map(
          (doc) => ({ id: doc.id, ...doc.data() }) as Aula,
        ),
      );
    } catch (error) {
      console.error(error);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingItem) {
        await updateDoc(
          doc(db, "treinamentos", trainingId!, "aulas", editingItem.id),
          {
            ...formData,
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(
        doc(db, "treinamentos", trainingId, "aulas", deletingItem.id),
      );
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6">
        <div>
          ...
          <p className="text-gray-500 font-medium mt-2 italic px-1">
            Gerenciamento de aulas para este treinamento.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading
          ? [1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white h-64 animate-pulse rounded-[40px] border border-gray-100"
              />
            ))
          : lessons.map((item) => (
              <motion.div
              ...
              >
                ...
                <p className="text-gray-500 text-sm font-medium line-clamp-3 italic">
                  {item.descricao}
                </p>
              </motion.div>
            ))}
          ...
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-gray-500 font-medium italic mb-8">
                Tem certeza que deseja apagar a aula{" "}
